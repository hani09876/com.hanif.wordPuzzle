...
func _ready():
	...
	for i in group.get_buttons():
		...
		if count < lavel:
			i.icon = preload("res://images/logo/lav.png")
			count += 1
...
func onpressed():
	...
	if click > lavel:
		...
		$Tween.interpolate_property($popup, "scale",Vector2(0, 0), Vector2(1.2,1.2),1,Tween.TRANS_ELASTIC)
		$Tween.start()
	elif lavel == click:
		StoreData.now = 1 
...
func _on_close_pressed():
	$Tween.interpolate_property($popup, "scale",Vector2(1.2, 1.2), Vector2(0,0),1,Tween.TRANS_ELASTIC)
	$Tween.start()
